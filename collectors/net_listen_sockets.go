...
func CollectListeningSockets() []model.ListenSocket {
	...
	var result []model.ListenSocket

	fds, _ := globFn("/proc/[0-9]*/fd/*") // поиск процессов, держащих сокеты/файлы
...
func parsePort(hexAddr string) int { // достаём порт
	p := strings.Split(hexAddr, ":")
	if len(p) != 2 {
		return 0
	}
	port, _ := strconv.ParseInt(p[1], 16, 32)
	return int(port)
}
