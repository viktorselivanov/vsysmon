...
func (c *TopTalkerCollector) Collect(s *model.Sample) {
	...
	var protoTop []model.ProtoTalker
...
func readNetFile(path, proto string, protoMap map[string]uint64, flowMap map[string]uint64) {
	f, err := os.Open(path)
	if err != nil {
		return
	}
	...
	for sc.Scan() {
		...
		local := fields[1]
		remote := fields[2]
		txrx, _ := strconv.ParseUint(fields[9], 10, 64) // собираем объём информации в байтах
...
func parseAddr(hex string) string {
	...
	for i := 0; i < 4; i++ {
		b, _ := strconv.ParseInt(ipHex[i*2:i*2+2], 16, 64)
		ip[3-i] = strconv.Itoa(int(b))
	}

	port, _ := strconv.ParseInt(portHex, 16, 64) // переводим hex
