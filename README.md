...
Удобная конфигурация запуска и отображения метрик через конфиг файл config.json*

\*   В случае отсутсвия config.json при запуске агента используется default config со всеми метриками
    Для клиента всегда используется default config, в случае отключения метрик (в агенте) будут отображены нулевые значения соответственно
