...
var readDiskStat = func() diskStat {
	...
	if err != nil {
		// если не можем открыть, возвращаем пустую статистику
		return diskStat{}
	}
	...
	for sc.Scan() {
		...
		if strings.HasPrefix(fs[2], "sd") || strings.HasPrefix(fs[2], "nvme") {
			rd, err := strconv.ParseUint(fs[3], 10, 64)
			if err != nil {
				continue
			}
			wr, err := strconv.ParseUint(fs[7], 10, 64)
			if err != nil {
				continue
			}
			sec, err := strconv.ParseUint(fs[5], 10, 64)
			if err != nil {
				continue
			}
			wsec, err := strconv.ParseUint(fs[9], 10, 64)
			if err != nil {
				continue
			}
